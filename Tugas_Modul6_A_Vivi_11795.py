...
model = load_model(r"C:\Users\ASUS\Documents\PMDPM\Tugas6_A_11795\model_mobilenet.h5")
...
def custom_progress_bar(confidence, color1, color2):
    percentage0 = confidence[0] * 100
    percentage1 = confidence[1] * 100
    progress_html = f"""
    <div style="border: 1px solid #ddd; border-radius: 5px; overflow: hidden; width: 100%; font-size: 14px;">
        <div style="width: {percentage0:.2f}%; background: {color1}; color: white; text-align: center; height: 24px; float: left;">
            {percentage0:.2f}%
        </div>
        <div style="width: {percentage1:.2f}%; background: {color2}; color: white; text-align: center; height: 24px; float: left;">
            {percentage1:.2f}%
        </div>
    </div>
    """
...
st.title("Prediksi Kematangan Buah Naga - 1795")
